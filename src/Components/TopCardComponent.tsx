...
const TopCardComponent = (prop: {card:string, company:string, posNeg:string, topBorder:string, logo:string, username:string, headText:string, followers:string, type:string, statsColor:string, statLogo:string, stats:string}) => {
  return (
    <div>
      <div className={`${prop.card} text-center rounded-[5px] bg-[#f0f2fa] dark:bg-[#252a41]`}>
        ...
        <h1 className={`${prop.headText} font-bold  text-[50px] dark:text-white leading-none`}>{prop.followers}</h1>
